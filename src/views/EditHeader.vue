<template>
	<section>
		...
		<div class="container">
			...
			<div class="col">
				<form method="POST" enctype="multipart/form-data"  @submit.prevent="editHeader">
					<div class="control">
						<input type="text" class="input" placeholder="Type Your Website Title" v-model="title" autocomplete="off">
					</div>
					<div class="control">
						<textarea class="input" placeholder="Type Your Website Subtitle" v-model="subtitle" autocomplete="off"></textarea>
					</div>
					<div class="control">
	...
	export default {
		...
		methods: {
			getHeader: async function() {
				const result = await HTTP.connect('get', '/header', null, null);
				this.title = result.result.title;
				this.subtitle = result.result.subtitle;
			},
			editHeader: async function() {
				const formData = new FormData();
				formData.append('title', this.title);
				formData.append('subtitle', this.subtitle);
				const result = await HTTP.connect('put', '/header', formData, this.$session.get('token'));
